...
class DatabaseManager:
            ...
            try:
                with conn.cursor() as cursor:
                    ...
                    self._log_audit_event(
                        ...
                        record_id=run_id,
                        new_values={'processor': processor, 'run_date': date.isoformat()}
                    )
                    conn.commit()
                    return run_id
            except Exception:
    ...
    def store_reconciliation_result(self, run_id: str, result: ReconciliationResult) -> None:
        """
    ...
    def update_reconciliation_status(self, run_id: str, status: str, error_message: Optional[str] = None):
        """
        ...
        with self.get_connection() as conn:
            ...
            try:
                with conn.cursor() as cursor:
                    if status == 'failed':
                        cursor.execute("""
                            ...
                            SET status = %s, 
                                ...
                                updated_at = CURRENT_TIMESTAMP
                            WHERE id = %s
                        """, (status, datetime.utcnow(), error_message or "Unknown error", run_id))
                    else:
                        cursor.execute("""
                            UPDATE reconciliation_runs 
                    ...
                    conn.commit()
                    logger.warning("Updated run status", run_id=run_id, new_status=status)
            except Exception:
                raise
