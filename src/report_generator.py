...
class ReportGenerator:
    ...
    def _generate_detailed_csv(self, result: ReconciliationResult, output_dir: Path) -> Path:
        ...
        if not data:
            ...
            df = pd.DataFrame(columns=[field for field in Transaction.__annotations__.keys()])
        else:
...
"""
    ...
    def _generate_recommendations(self, result: ReconciliationResult) -> str:
        ...
        if result.summary.missing_transactions_count > 0:
            recommendations.append("• Review and reprocess missing transactions within 24 hours")
        else:
